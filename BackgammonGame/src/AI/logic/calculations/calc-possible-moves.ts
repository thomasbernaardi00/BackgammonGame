...
export function hasPossibleMove(game: Game, thisTurn: ThisTurn): boolean {
  ...
  containing.forEach((barIdx) => {
    const canGoTo = calcPossibleMoves(game, barIdx, thisTurn);

    canGoTo.forEach((barIdx) => allMoves.push(barIdx));
  });

  const endingDiceBars = calcEndingDiceBars(game, thisTurn);
  endingDiceBars.forEach((barIdx) => allMoves.push(barIdx));
...
export function calcPossibleMoves(
  ...
  console.log(`Calcolando le mosse per la barra ${fromBarIdx}`);
  console.log(`Dadi: ${firstDice}, ${secondDice}`);
  ...
  for (let i = 0; i < game.board.length; i++) {
    ...
    if (toBar.includes(thisTurn.opponentPlayer.name) && toBar.length > 1) {
      continue; // Non può muovere su una barra piena di pedine dell'avversario
    }
    ...
    if (thisTurn.turnPlayer.name === "White") {
      if (
        ...
        continue; // Movimento non valido per il giocatore White
      }
    } else {
      if (
        (fromBarIdx <= 11 && toBarIdx <= 11 && toBarIdx <= fromBarIdx) ||
        ...
        continue; // Movimento non valido per il giocatore Black
      }
    }
    ...
    console.log(`Distanza tra ${fromBarIdx} e ${toBarIdx}: ${distance}`);

    if (distance === 0 || (distance !== firstDice && distance !== secondDice)) {
      continue; // Se la distanza non è uguale ai dadi, la mossa non è valida
    }

    canGoTo.push(toBarIdx);
  }

  console.log(`Mosse valide: ${canGoTo}`);
  return canGoTo;
}
