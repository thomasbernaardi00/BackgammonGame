...
export function checkCantMove(game: Game, thisTurn: ThisTurn, io: Server, gameid: string): ThisTurn {
...
export function calcGettingOutOfOutMoves(
  ...
  function checkForOpponent(bar: number) {
    ...
    return (
      ...
      (game._board[bar].includes(opponentPlayer) && game._board[bar].length === 1)
    );
  }
